...
### アップデート通知機能
- **ダッシュボード通知**: ログイン時に新しいアップデート情報を自動表示
- **ワンタイム表示**: 一度確認したアップデートは自動的に非表示
- **詳細な変更履歴**: 新機能・改善点・バグ修正を分類して表示
- **履歴管理**: 設定ページから過去のアップデート履歴を確認可能
...
│   │   │   ├── request/     # シフト希望ページ
│   │   │   └── view/        # シフト表示・編集ページ
│   │   └── shift-requests/  # シフト希望管理ページ
│   ├── components/          # 再利用可能コンポーネント
│   │   ├── auth/            # 認証コンポーネント
│   │   │   ├── AuthForm.tsx          # 認証フォーム
│   │   │   └── GoogleSignInButton.tsx # Google認証ボタン
│   │   ├── layout/          # レイアウトコンポーネント
│   │   │   ├── Layout.tsx   # メインレイアウト
│   │   │   └── Sidebar.tsx  # サイドバー
│   │   ├── shift/           # シフト関連コンポーネント
...
│   │   ├── ai/              # AI関連コンポーネント
│   │   │   └── AIDataManagement.tsx # AIデータ管理
│   │   ├── GanttChart.tsx   # ガントチャートコンポーネント
│   │   ├── LocalFileManager.tsx # ローカルファイル管理
│   │   └── ShiftPaymentCalculator.tsx # 給与計算
│   ├── lib/                 # ライブラリとユーティリティ
│   │   ├── firebase.ts      # Firebase設定
│   │   ├── shiftOptimizer.ts        # シフト最適化ロジック
│   │   ├── aiShiftOptimizer.ts      # AI駆動シフト最適化
│   │   ├── shiftService.ts          # シフトサービス
│   │   ├── shiftTimeUtils.ts        # 時間計算ユーティリティ
│   │   ├── shiftValidator.ts        # シフトバリデーション
│   │   ├── asmitoFileManager.ts     # ファイル管理
│   │   ├── aiDataManager.ts         # AIデータ管理
│   │   ├── aiDataFirebaseService.ts # AIデータFirebaseサービス
│   │   └── utils.ts         # 共通ユーティリティ
│   ├── services/            # API サービス
│   │   ├── employeeService.ts      # 従業員サービス
│   │   ├── shiftRequestService.ts  # シフト希望サービス
│   │   ├── dataManagementService.ts # データ管理サービス
│   │   ├── shiftAIService.ts       # シフトAIサービス
│   │   └── shiftAIFreeService.ts   # 無料版AIサービス
│   ├── store/               # Zustand状態管理
│   │   ├── authStore.ts     # 認証状態
│   │   └── themeStore.ts    # テーマ状態
│   └── types/               # TypeScript型定義
...
### v0.3.2 (2025年7月31日) - プロジェクト構造とドキュメント更新
- README.mdの全面的な更新と整理
- プロジェクト構造の詳細化
- AIデータ管理機能の強化
- 開発環境の最適化
